...
df_r[is.na(df_r)] <- 0
df_r$sample <- gsub("^", "X", df_r$sample)

### rename columns
names(df_r)[names(df_r) == 'sample'] <- 'specific_contig_source'
names(df_r)[names(df_r) == 'OTU'] <- 'kaiju_taxonomy'
names(df_r)[names(df_r) == 'count'] <- 'raw_counts'


taxa_obj_CSS_HM_c <- merge(df_r, taxa_obj_CSS_HM_c, by.x=c("specific_contig_source","kaiju_taxonomy"), by.y=c("specific_contig_source","kaiju_taxonomy"), all.y=TRUE)
...
taxa_obj_CSS_HM_c %>%
filter(grepl('01FEB24DM1', seqrun)) %>%
ggplot(aes(x=specific_contig_source, y=kaiju_taxonomy, fill=counts)) + 
  geom_tile() +
  scale_fill_viridis(option="inferno", limits = c(0, 1000), oob = scales::oob_squish) +
...
df_merged_cat$seqrun <- gsub("__barcode.*", "", df_merged_cat$contig)
...
filter(!grepl('Alphaflexiviridae', kaiju_taxonomy_higher)) %>%
filter(grepl('nege_like_virus|Allermuir|Triatovirus|Iflavirus|Lake_Sinai_virus', kaiju_taxonomy)) %>%
ggplot(aes(x=percent_of_contig_len_with_over_nX_coverage, y=contig_length)) +
geom_jitter(aes(color = genus), alpha = 0.5, size = 1) + 
facet_nested(kaiju_taxonomy_higher + kaiju_taxonomy ~ seqrun, switch = "both", scales = "free_y") +
theme(strip.text.y.left = element_text(angle = 0, hjust = 1)) +
theme(strip.background = element_blank()) +
theme(panel.border = element_blank()) +
theme(axis.title.y = element_blank()) +
...
facet_nested(kaiju_taxonomy_higher + kaiju_taxonomy ~ seqrun, switch = "y", space = "free_y") +
theme(strip.text.y.left = element_text(angle = 0, hjust = 1)) +
theme(strip.background = element_blank()) +
theme(panel.border = element_blank()) +
theme(axis.title.y = element_blank()) +
...
df <- read_sheet("https://docs.google.com/spreadsheets/d/1BMMCJ6y7Slz9MeGH-5rkZFFZlx-c3Mof4L2fDmdkPAY/edit#gid=0", sheet="All_data")
